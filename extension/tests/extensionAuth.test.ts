...
import {
  buildWebLoginUrl,
  handleExternalAuthMessage,
  type ExtensionAuthStorage
} from '../src/shared/auth/extensionAuth'
...
describe('extensionAuth', () => {
  it('opens the web login flow with an extension connect redirect', () => {
    ...
    expect(url.toString()).toBe(
      'http://localhost:5173/?redirect=%2Fextension%2Fconnect%3FsourceUrl%3Dhttps%253A%252F%252Fwww.jasoseol.com%252Frecruit%252F1'
    )
  })

  it('stores the web-issued EZ One session sent by the extension connect page', async () => {
    const storage: ExtensionAuthStorage = {
