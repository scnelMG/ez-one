...
ALLOW_DIRTY_ROLLBACK="${ALLOW_DIRTY_ROLLBACK:-false}"
...
verify_release_manifest() {
  ...
  verify_release_artifact_names "$manifest_release_id"
  manifest_worktree="$(require_manifest_value git_worktree)"
  if [[ "$manifest_worktree" == "dirty" && "$ALLOW_DIRTY_ROLLBACK" != "true" ]]; then
    fail "RELEASE-MANIFEST.txt has git_worktree=dirty. Use a clean previous release or set ALLOW_DIRTY_ROLLBACK=true only for rehearsal rollback tests."
  fi
...
if [[ "$ALLOW_DIRTY_ROLLBACK" != "true" && "$ALLOW_DIRTY_ROLLBACK" != "false" ]]; then
  fail "ALLOW_DIRTY_ROLLBACK must be true or false"
fi
...
if [[ -z "$BACKEND_ARTIFACT" || -z "$FRONTEND_ARTIFACT" || -z "$EXTENSION_ARTIFACT" || -z "$RELEASE_MANIFEST" || -z "$CHECKSUM_FILE" ]]; then
  fail "Rollback requires BACKEND_ARTIFACT, FRONTEND_ARTIFACT, EXTENSION_ARTIFACT, RELEASE_MANIFEST, and CHECKSUM_FILE"
fi
...
pass "backend rollback target checked for $SERVICE_NAME"
...
pass "frontend rollback target checked for $FRONTEND_TARGET"

pass "extension rollback artifact checked: $EXTENSION_ARTIFACT"
...
if [[ -n "$BASE_URL" && "$DRY_RUN" == "false" ]]; then
  ...
  printf '[RUN] curl --fail --silent --show-error --max-time 20 %s/api/health\n' "$BASE_URL"
fi
run_runtime_check
