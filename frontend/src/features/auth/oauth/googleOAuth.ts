...
interface GoogleOAuthUrlParams {
  clientId: string
  redirectUri: string
  state: string
}

export function createOAuthState(redirectPath: string) {
...
export function consumeOAuthState(nonce: string) {
  ...
  const state = JSON.parse(rawState) as StoredOAuthState
...
export function buildGoogleOAuthUrl({ clientId, redirectUri, state }: GoogleOAuthUrlParams) {
