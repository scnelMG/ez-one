...
describe('MyPage', () => {
    ...
    it('MY-002/MY-003/PROFILE-025: edits onboarding preference fields from my page', async () => {
        ...
        await flushPromises();
        expect(wrapper.get('[data-testid="profile-desired-roles"]').element.value)
            .toBe('Backend Developer');
        expect(wrapper.get('[data-testid="profile-company-types"]').element.value)
            .toBe('Startup');
        expect(wrapper.get('[data-testid="profile-regions"]').element.value)
            .toBe('Seoul');
        expect(wrapper.get('[data-testid="profile-skills"]').element.value)
