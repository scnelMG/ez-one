...
@TestPropertySource(properties = {
    "app.cors.allowed-origins=http://localhost:5173,chrome-extension://ikpeibohnopmikegoogggmdipmhmiadi"
})
class AuthControllerTest {
    ...
    @Test
    void extensionApiRejectsUnlistedChromeExtensionCorsPreflight() throws Exception {
        mockMvc.perform(options("/api/extension/jobs/save")
    ...
    void googleLoginReturnsTokenEnvelope() throws Exception {
        ...
        mockMvc.perform(post("/api/auth/google")
            ...
            .andExpect(jsonPath("$.data.refreshToken").value(Matchers.nullValue()))
            ...
            .andExpect(header().string(HttpHeaders.SET_COOKIE, Matchers.allOf(
                Matchers.containsString("ezone_refresh_token=refresh-token"),
    ...
    void refreshAndLogoutReturnApiEnvelope() throws Exception {
        ...
        mockMvc.perform(post("/api/auth/refresh")
            ...
            .andExpect(jsonPath("$.data.tokenType").value("Bearer"))
            .andExpect(header().string(HttpHeaders.SET_COOKIE, Matchers.containsString("ezone_refresh_token=new-refresh-token")));
    ...
    @Test
    void refreshAcceptsHttpOnlyCookieWhenBodyIsMissing() throws Exception {
        when(authService.refresh(any())).thenReturn(
            new AuthTokenResponse(
                "new-access-token",
                "new-refresh-token",
        ...
        mockMvc.perform(post("/api/auth/refresh")
                .cookie(new Cookie(AuthController.REFRESH_COOKIE_NAME, "cookie-refresh-token")))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.data.accessToken").value("new-access-token"))
            .andExpect(jsonPath("$.data.refreshToken").value(Matchers.nullValue()))
            .andExpect(header().string(HttpHeaders.SET_COOKIE, Matchers.containsString("ezone_refresh_token=new-refresh-token")));
        ...
        org.assertj.core.api.Assertions.assertThat(captor.getValue().refreshToken()).isEqualTo("cookie-refresh-token");
    }
