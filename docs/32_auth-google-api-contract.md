...
The frontend stores OAuth `state` by nonce with a short-lived storage map. Session storage is primary, local storage is used as a same-origin backup for slower Google account handoffs, and entries expire after 30 minutes. This allows repeated attempts or parallel login tabs without overwriting the previous in-flight state. Google error callbacks such as `access_denied` are handled on the callback page and must not call the backend token exchange endpoint.
